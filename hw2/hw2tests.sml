...
val test1_edit =  case make_silly_json 3 of
        Array [Object [("n",Num x),
                       ("b",True)],
	       Object [("n",Num y),
                       ("b",True)],
               Object [("n",Num z),
                       ("b",True)]]
        => check_real (x,3.0) andalso check_real(y,2.0) andalso check_real (z,1.0)
		   | _ => false
val test2 = assoc ("foo", [("bar",17),("foo",19)]) = SOME 19

val test2_edit = assoc ("bar", [("bar",17),("foo",19)]) = SOME 17
							       
val test3 = case dot (json_obj, "ok") of SOME True => true |  _ => false

val test3_edit = case dot (json_obj, "k") of NONE => true | _ => false
								     
val test4 = one_fields json_obj = rev ["foo","bar","ok"]
				      
val test4_edit = one_fields (Object[("a", True), ("b", False),("c", Num 1.0)]) = ["c","b","a"]

val test5 = not (no_repeats ["foo","bar","foo"])

val test5_edit = no_repeats ["a","b","c"]
